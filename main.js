const CONFIG = {
  NAME: "Export Gmail Threads as PDFs",
  RANGE_NAME: {
    QUERY: "query",
    MAX: "max",
...
const createThreadContent_ = (thread, printMessagesOnly = false) => {
  ...
  if (imageLinks.length) {
    ...
    requests.forEach((req, index) => {
      try {
        const response = UrlFetchApp.fetch(req.url);
        const blob = response.getBlob();
        const uri = _toBase64ImageUri_(blob);
        htmlBody = htmlBody.replace(new RegExp(req.url, "g"), uri);
      } catch (err) {
        console.warn(`Failed to fetch image URL: ${req.url} - ${err.message}`);
      }
    });
  }
  return htmlBody;
...
const print_ = () => {
  ...
  if (merge) {
    const html = threads
      ...
      .join("<br/>");
    const pdf = htmlToPdf_(html, `${filename}.pdf`);
